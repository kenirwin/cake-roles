    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= h($role->role) ?></td>
                <td><?= $this->Number->format($role->start_year) ?></td>
                <td><?= $this->Number->format($role->end_year) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $role->woman_id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $role->woman_id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $role->woman_id], ['confirm' => __('Are you sure you want to delete # {0}?', $role->woman_id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
